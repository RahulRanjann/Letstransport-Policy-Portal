      ...
      onFinish: async (completion) => {
        if (completion.text && completion.text.includes("I don't have that information. I've sent your question to HR")) {
          ...
          await resend.emails.send({
            ...
            html: `<div><h2>Policy Question from Employee</h2><div><h3>User Details:</h3><p><strong>Name:</strong> ${userName || 'Not available'}</p><p><strong>Email:</strong> ${userEmail}</p></div><div><h3>Question:</h3><p>"${lastUserMessage}"</p></div><div><p><strong>Action Required:</strong> Please respond directly to the user at <a href="mailto:${userEmail}">${userEmail}</a> with the appropriate information.</p></div><div><p>This email was automatically generated by the LetsTransport Policy Assistant.</p></div></div>`,
