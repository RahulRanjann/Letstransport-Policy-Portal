  ...
  try {
    ...
    if (lastUserMessage) {
      ...
      if (darwinboxAnswer && !darwinboxAnswer.startsWith("I can help you with Darwinbox-related questions like:")) {
        ...
        // Check if this is a question that should trigger HR email
        if (darwinboxAnswer.includes("I don't have that information. I've sent your question to HR")) {
          // For HR email questions, send the email immediately and return the response
          console.log('Darwinbox answer triggered HR email for question:', lastUserMessage?.substring(0, 100) + '...');
          ...
          // Return a friendly version of the HR response
          const result = await streamText({
            model: google("gemini-1.5-flash"),
            prompt: `You are Sarah, an HR assistant at LetsTransport. The user asked about their manager/reporting structure. You need to tell them that you've sent their question to HR and they'll get back to them directly. Be friendly and reassuring. Keep it short and conversational.`,
            temperature: 0.7,
            maxTokens: 100,
          })
          return result.toDataStreamResponse()
        ...
        // For other Darwinbox answers, proceed normally
        const result = await streamText({
          model: google("gemini-1.5-flash"),
